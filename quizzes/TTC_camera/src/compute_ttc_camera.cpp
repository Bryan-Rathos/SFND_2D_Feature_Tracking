...
{
    cout << kptsPrev.size() << " " << kptsCurr.size() << " " << kptMatches.size() << endl;
    ...
    if(size%2 == 0)
        medianDistRatio = (distRatios[(size-1)/2] + distRatios[size/2]) / 2;
    else
...
int main()
{
    ...
    for(auto& ele : matches){
        cout << ele.queryIdx << ", " << ele.trainIdx << ", " << ele.imgIdx << ", " << ele.distance << endl;
    }
